...
| Solana devnet lifecycle | e2e-devnet | A 0.01 SOL payment from a direct treasury on `solana:devnet`: policy check, real simulation, approval, sign-before-broadcast, `finalized` commitment, ledger, and reconciliation against the destination delta. Settled in transaction `3vgtSKuLwo1eUQ68zpBYPMbQHnvexcpAQmQ6AXDK9itGzpXFYbKYk5XhBQ4aqkfqdbRNteB29mwQgcxDaYJopXk2` on 2026-09-17 (`apps/worker/src/devnet.test.ts`, `RUN_DEVNET=1`). Still required: Squads and x402 on devnet, provider failover |
| Squads on devnet | e2e-devnet | A Squads v4 vault created on `solana:devnet` against the real program, with the Relay executor holding Initiate and Execute only: the executor published the vault transaction and proposal, the member voted on chain, the executor executed, and the payment reconciled. Vault `EC2efA35YePj9BJ2WJQ3J2HBi7E3bfTKtZrw9cteS32Q`, transaction `5Z3wWWqZbHiaHfhcaMJFJwCc63tFhGNbwrFNCXTazFFEK3AttgL7Ss6Pb7CQqENe69bCwduaAq5TJDuU7ig57hom`, 2026-09-17 |
| x402 on devnet | e2e-devnet | x402 v2 `exact` on `solana:devnet`: the seller quoted 0.125 TUSD, intake captured the quote as approval evidence, the worker signed the payer-side transfer after approval, a facilitator settled it on devnet, and the payee's balance matched at `finalized`. Mint `69XnUCzBdLFCKLP2NQRd1eiFYcVArB9wKvDyeCjoeLoj`, settlement `3QX4RzKQevh6ijhXzUj9YpDBExAbGimQGdk6hKuP2S3zyzmYnC5CVQMhKTvffbKXTPYTTTu2a8b6LWJ6dQqQuwhD`. The facilitator here is Relay's own local one pointed at devnet; the hosted CDP facilitator is still untested |
| Browser wallet path | e2e-local | The console driven by a foreign EIP-1193 provider injected at document start, not the in-page development wallet: `window.ethereum` detected, SIWE sign-in, a payment approved with that wallet's signature, and settlement reconciled on the local chain. A packaged extension could not be loaded (Chrome 153 ignores `--load-extension`), so MetaMask, Rabby, and Phantom themselves remain untested |
| Base Sepolia lifecycle | researched | Preflight is green: RPC reachable at chain 84532, canonical Safe 1.4.1 singleton and factory deployed, USDC at 0x036CbD53842c5426634e7929541eC2318f3dCF7e with 6 decimals. The transfer test is written and skips until `DEVNET_EVM_KEY` holds a funded key, because Base Sepolia faucets are gated behind an account |
...
| x402 from a Safe | e2e-local | A Safe with no signing key of its own pays an x402 resource: the authorization is fixed at publication with a nonce derived from the intent, owners sign an EIP-712 Safe message over its digest, their signatures are assembled into the Safe's ERC-1271 signature, and the facilitator settles through the token's bytes-signature `transferWithAuthorization`. Proven for a 2-of-2 Safe (`apps/worker/src/safe-x402.test.ts`) and a 1-of-1 Safe over HTTP including the approval endpoints (`apps/api/src/governance.test.ts`), and driven from the console against a local seller. A one-owner Safe's signature is padded past 65 bytes so a verifier cannot mistake it for an EOA signature. Still required: a public network with real USDC (2.2 or later; older USDC does not accept contract signatures), and a hosted facilitator |
