...
|---|---|---|
| achi-b6 (Claude, takeover) | everything under this repo | **done:** wallet auth, agent keys, policy/freeze/quorum on the durable path, sign-before-broadcast recovery, real finality, fee ledger, ERC-20/SPL. Safe + Squads on the intent path (executor never votes), x402 v2 from direct treasuries, invoices/beneficiaries/recurring/statements with a chain indexer and reconciliation, KMS custody and key/executor rotation. Web console on the real API (every page, public pay page, dev wallets), verified in headless Chrome. **next:** devnet runs, failure-recovery certification, CI |
