...
const devnet = process.env.RUN_DEVNET === "1" && databaseUrl ? it : it.skip;
...
describe("public devnets", () => {
  devnet("settles a SOL transfer on Solana devnet at finalized commitment", async () => {
    ...
    try {
      ...
      const ws = await workspace(store, "svm", solNetwork, assetId);
      ...
      if (balance < 0.05 * LAMPORTS_PER_SOL) {
        ...
        if (!signature) throw new Error(`The devnet faucet refused. Fund ${ws.treasury.address} with a little SOL and run again.`);
  ...
  devnet.skipIf(!evmKey)("settles an ETH transfer on Base Sepolia at the configured confirmation depth", async () => {
    const store = createPostgresStore(databaseUrl!);
    ...
    try {
      ...
      if (balance < parseEther("0.002")) throw new Error(`Fund ${ws.treasury.address} with Base Sepolia ETH (has ${balance})`);
